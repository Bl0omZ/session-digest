...
读取 Claude Code、Codex、Cursor、Grok、Pi 的会话记录，去掉无效 ID 和系统注入，输出干净的 Markdown。省 token，跨 agent 快速恢复上下文。
...
| Grok | `~/.grok/sessions/<URL编码cwd>/<id>/chat_history.jsonl` | `system` / `reasoning`；`<system-reminder>` / `<user_info>` / `<git_status>` 注入 |
